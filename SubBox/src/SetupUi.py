...
class SetupUi(QtGui.QDialog):
    ...
    #componenti terza pagina - selezione del percorso della cartella SubBox
        ...
        out_file = open("/imposta1.txt","w")
        out_file.write("imposta1")
        out_file.close()

        self.creaSetupPagina()

        out_file = open("/imposta2.txt","w")
        out_file.write("imposta")
        out_file.close()

        self.creaAccountPagina()
        
        out_file = open("/imposta3.txt","w")
        out_file.write("imposta")
        out_file.close()

        self.creaSelPercorsoPagina()

        out_file = open("/imposta.txt","w")
        out_file.write("imposta")
        out_file.close()

        self.impostaPagina()

        out_file = open("/imposta4.txt","w")
        out_file.write("imposta")
        out_file.close()
    ...
    def impostaPagina(self):
        ...
        if self.paginaCorrente == 1:
            ...
            if self.rbSceltaPercorso.isChecked() == True:
                self.acc.setPercorso(self.lbPercorsoHome.text().append("/SubBox"))
                createDir =  QtCore.QDir( self.lbPercorsoHome.text() )
            ...
            out_file = open("/acc.txt","w")

            out_file.write(self.acc.nome)
            out_file.write(self.acc.cognome)
            out_file.write(self.acc.email)
            out_file.write(self.acc.password)
            out_file.write(self.acc.percorso)
    ...
    def creaDatabase(self):
        ...
        connessioneDb = sqlite3.connect(pathDb)
        cursoreDb = connessioneDb.cursor()

        try:

            cursoreDb.execute("CREATE TABLE LOGIN (idLogin INTEGER PRIMARY KEY, nome TEXT, cognome TEXT, email TEXT, password TEXT, percorso TEXT); ")

        except sqlite3.OperationalError, msg:
            out_file = open("/Errori.txt","w")
            out_file.write(msg.args[0])
            out_file.close()

        try:

            cursoreDb.execute('INSERT INTO LOGIN (idLogin,nome,cognome,email,password,percorso) values (?,?,?,?,?,?);', \
            [0, unicode(self.acc.nome) ,unicode(self.acc.cognome) ,unicode(self.acc.email) ,unicode(self.acc.password), unicode(self.acc.percorso) ] )

        except sqlite3.OperationalError, msg:
            out_file = open("/Errori2.txt","w")
            out_file.write(msg.args[0])
            out_file.close()
